# Regexes for commands that refer to other lists of some sort ("pointers")
#
# NOTE THAT THESE ARE DYNAMICALLY CONSTRUCTED FOR EACH INPUT FILE BASED ON THE
...
'acl' => {
    ...
    11 => qr /^ \s*
                ...
                (?: \s+ | $)
                /ixsm,
    #BUG TODO Make ACL a list because BGP can match multiple ACLs
    12 => qr /^ \s*
                match                   \s+
                ip                      \s+
                address                 \s+
...
4 => qr/^ \s*
        route-map                   \s+
        (?<points_to> $list_of_pointees_ref->{"route_map"} ) \s+
        (?: in | out)
        \s* $
        /ixsm,
...
'community_list' => {
    '1_list' => qr/^ \s*
                    ...
                    (?<points_to> (?: $list_of_pointees_ref->{"community_list"} | \s )+ )       #This can be a list of things
                                                                                                #separated by whitespace
                /ixsm,
},
'as_path_access_list' => {
...
'ios_bgp_peer_group' => {
    ...
    1 => qr/
            peer-group              \s+
            (?<points_to> (?: $list_of_pointees_ref->{"ios_bgp_peer_group"}) )
            \s* $
            /ixsm,
},
'nxos_flow_exporter' => {
    ...
    1 => qr/ ^ \s*
            exporter    \s+
            (?<points_to> (?: $list_of_pointees_ref->{"nxos_flow_exporter"}) )
            (?: \s* | $)
            /ixsm,
},
'nxos_flow_monitor' => {
    ...
    1 => qr/ ^ \s*
            ...
            flow        \s+
            monitor     \s+
            (?<points_to> (?: $list_of_pointees_ref->{"nxos_flow_monitor"}) ) \s+
            /ixsm,
},
'nxos_flow_record' => {
    # Added 2017-07-29
    1 => qr/ ^ \s*
            record  \s+
            (?<points_to> (?: $list_of_pointees_ref->{"nxos_flow_record"}) )
            \s* $
            /ixsm,
},
'ios_voice_port' => {
    ...
    1 => qr/ ^ \s*
            port \s+
            (?<points_to> (?: $list_of_pointees_ref->{"ios_voice_port"}) )
            \s* $
            /ixsm,
},
